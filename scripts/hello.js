    ...
    on_message: function (message) {
        ...
        if (body) {
            ...
            $('#chat-' + jid_id + ' .chat-messages').append(
                ...
                Strophe.getNodeFromJid(jid)+"</b>" +
                "</span>"+"("+timeNow+")"+"<br/><span class='chat-text'>" +
                "</span></div>");
...
/*-------------------------------------------------------------*/
       $('#chatroom_dialog').dialog({
        ...
        buttons: {
            "Create": function () {
                $(document).trigger('chatroom', {
                    room: $('#chatroom-jid').val(),
                    owner: $('#owner-jid').val().toLowerCase()
                });
    ...
    $('#chat-room').click(function (ev) {
        $('#chatroom_dialog').dialog('open');
    });
/*-------------------------------------------------------------*/
    ...
    $('.chat-input').live('keypress', function (ev) {
        ...
        if (ev.which === 13) {
            ...
            $(this).parent().find('.chat-messages').append(
                ...
                "<b>"+Strophe.getNodeFromJid(Gab.connection.jid)+"</b>" +
                "</span>"+" ("+timeNow+")"+"<br/><span class='chat-text'>" +
                body +
                "</span></div>");
...
$(document).bind('connect', function (ev, data) {
    var conn = new Strophe.Connection(
        'http://34.192.95.107:5280/http-bind');
...
/*------------------------------------------------------------------------*/
$(document).bind('chatroom', function (ev, data) {
    var chatroom = $pres({to: data.room, from:data.owner}).c("x", {xmlns: 'http://jabber.org/protocol/muc'}) ;
    Gab.connection.send(chatroom);
});